const juego = (function () {
  ...
  function ganador() {
    const divCasilleroAremoverListener =
      document.querySelectorAll(".casillero");
    for (let casillero = 0; casillero < arregloTablero.length; casillero++) {
      divCasilleroAremoverListener[casillero].removeEventListener(
        "click",
        jugadaHecha
      );
    }
  ...
  function jugadaHecha(event) {
    if (
      jugadorEnTurno.nombre !== "compu" &&
      jugadorEnTurno.nombre !== "Compu"
    ) {
      if (
        arregloTablero[event.target.dataset.casillero] === jugador1.marca ||
        arregloTablero[event.target.dataset.casillero] === jugador2.marca
      ) {
        return;
      } else {
    ...
    for (let i = 0; i < arregloDarreglos.length; i++) {
      if (
        (arregloTablero[arregloDarreglos[i][0]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][1]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][2]] === jugadorEnTurno.marca) ||
        (arregloTablero[arregloDarreglos[i][0]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][2]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][1]] === jugadorEnTurno.marca) ||
        (arregloTablero[arregloDarreglos[i][1]] === jugadorEnTurno.marca &&
  ...
  function jugadaCompu() {
    ...
    for (let i = 0; i < arregloDarreglos.length; i++) {
      if (
        (arregloTablero[arregloDarreglos[i][0]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][1]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][2]] === "") ||
        (arregloTablero[arregloDarreglos[i][0]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][2]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][1]] === "") ||
        (arregloTablero[arregloDarreglos[i][1]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][2]] === jugadorEnTurno.marca &&
          arregloTablero[arregloDarreglos[i][0]] === "")
      ) {
        let indiceParaMarcar = [
          arregloTablero[arregloDarreglos[i][0]],
          arregloTablero[arregloDarreglos[i][1]],
          arregloTablero[arregloDarreglos[i][2]],
        ].indexOf("");
        arregloTablero[arregloDarreglos[i][indiceParaMarcar]] =
          jugadorEnTurno.marca;
        jugadaHecha();
    ...
    for (let i = 0; i < arregloDarreglos.length; i++) {
      if (
        (arregloTablero[arregloDarreglos[i][0]] === contrincanteDeCompu.marca &&
          arregloTablero[arregloDarreglos[i][1]] ===
            contrincanteDeCompu.marca &&
          arregloTablero[arregloDarreglos[i][2]] === "") ||
        (arregloTablero[arregloDarreglos[i][0]] === contrincanteDeCompu.marca &&
          arregloTablero[arregloDarreglos[i][2]] ===
            contrincanteDeCompu.marca &&
          arregloTablero[arregloDarreglos[i][1]] === "") ||
        (arregloTablero[arregloDarreglos[i][1]] === contrincanteDeCompu.marca &&
          arregloTablero[arregloDarreglos[i][2]] ===
            contrincanteDeCompu.marca &&
          arregloTablero[arregloDarreglos[i][0]] === "")
      ) {
        let indiceParaEvitar = [
          arregloTablero[arregloDarreglos[i][0]],
          arregloTablero[arregloDarreglos[i][1]],
          arregloTablero[arregloDarreglos[i][2]],
        ].indexOf("");
        arregloTablero[arregloDarreglos[i][indiceParaEvitar]] =
          jugadorEnTurno.marca;
        jugadaHecha();
    ...
    for (let i = 0; i < 1; i++) {
      for (let j = 0; j < 4; j++) {
        ...
        if (
          ...
          arregloTableroCopia[1] === contrincanteDeCompu.marca
        ) {
          if (
            arregloTableroCopia[3] === "" &&
            arregloTableroCopia[4] === "" &&
            arregloTableroCopia[6] === ""
          ) {
            arregloTableroCopia[6] = jugadorEnTurno.marca;

            if (j === 1) {
              arregloTableroCopia = rotarTableroIzq(
                rotarTableroIzq(rotarTableroIzq(arregloTableroCopia))
              );
            } else if (j === 2) {
              arregloTableroCopia = rotarTableroIzq(
                rotarTableroIzq(arregloTableroCopia)
              );
            } else if (j === 3) {
          ...
          if (
            arregloTableroCopia[4] === "" &&
            arregloTableroCopia[5] === "" &&
            arregloTableroCopia[8] === ""
          ) {
        ...
        if (
          ...
          if (
            arregloTableroCopia[6] === "" &&
            arregloTableroCopia[2] === "" &&
            arregloTableroCopia[0] === ""
          ) {
            arregloTableroCopia[0] = jugadorEnTurno.marca;
        ...
        if (
          arregloTableroCopia[0] === jugadorEnTurno.marca &&
          arregloTableroCopia[5] === jugadorEnTurno.marca
        ) {
          if (
            arregloTableroCopia[1] === "" &&
            arregloTableroCopia[8] === "" &&
            arregloTableroCopia[2] === ""
          ) {
            arregloTableroCopia[2] = jugadorEnTurno.marca;
        ...
        if (
          arregloTableroCopia[0] === jugadorEnTurno.marca &&
          arregloTableroCopia[4] === jugadorEnTurno.marca &&
          arregloTableroCopia[8] === contrincanteDeCompu.marca
        ) {
          if (
            arregloTableroCopia[7] === "" &&
            arregloTableroCopia[2] === "" &&
    ...
    arregloTablero[
      casillerosLibres[Math.floor(Math.random() * casillerosLibres.length)]
    ] = jugadorEnTurno.marca;
  ...
  function arrancar() {
    jugador1 = FabricaJugador(
      prompt("Nombre del jugador con marcas X?", "Jugador1"),
      "x"
    );
    jugador2 = FabricaJugador(
      prompt("Nombre del jugador con marcas O?", "Jugador2"),
      "o"
    );
    jugadorEnTurno = jugador1;
